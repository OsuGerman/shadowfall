# VELGRAD WORKFLOWS BIBEL (Lore-Referenz / Legacy)

> ⚠️ **STATUS (Update #171, 2026-05-24):** Procedural-Only-Pivot. Die Scenario.gg-Workflows sind **Legacy** — das Spiel rendert ausschliesslich procedural via [sf/sprites.py](sf/sprites.py).
>
> **Tools in [tools/workflow_*.py](tools/) bleiben** als Legacy fuer experimentelle One-Off-Generierungen, sind aber nicht mehr Teil des regulaeren Workflows. Falls jemand wieder Assets generieren will (z.B. fuer einen Asset-Pack-Mod), kann er diese Workflows nutzen UND `set_ai_sprites_enabled(True)` (siehe [sf/sprites.py](sf/sprites.py)).
>
> **Status:** v1.0 (2026-05-24) — gefroren
> **Scope (historisch):** Scenario.gg Standard-Library Workflows + Velgrad-spezifische Verwendung
