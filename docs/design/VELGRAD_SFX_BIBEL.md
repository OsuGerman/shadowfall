# VELGRAD — SFX-BIBEL (Lore-Referenz)

> ⚠️ **STATUS (Update #171, 2026-05-24):** Procedural-Pivot fuer Grafik (Sprites raus), Sounds bleiben aktuell aber werden bei Bedarf neu auf **Englisch** generiert (User: „deutsche AI-Stimmen klingen zu sehr nach KI").
>
> **Aktuelle Audio-Strategie:**
> - **Existierende MP3s in `Sounds/`** bleiben drin und werden via [sf/sounds.py](sf/sounds.py) abgespielt.
> - **Procedural-Synth-Builder** in [sf/sounds.py](sf/sounds.py) (Sub-Bass, Tremolo, Rise-Envelopes, Element-Signatures) sind die Foundation — viele SFX werden zur Laufzeit synthetisch erzeugt.
> - **SFX-Override-System** in [sf/sounds.py](sf/sounds.py): Wenn `<name>.{ogg,wav,mp3,flac}` in `Sounds/` existiert, wird die Datei statt des procedural-Synths benutzt. Keine Code-Aenderung noetig fuer Re-Generation.
>
> **Diese Bibel bleibt als Master-Katalog** der Soll-SFX (Welcher Sound braucht das Spiel, welche Tonalitaet, welcher Lore-Anker). Wenn die Englisch-Re-Generation startet, ist dies die Spec.
>
> **Tool-Strategie (historisch / fuer Re-Generation):**
...
Pro Mob aus [../gameplay/VELGRAD_BESTIARIUM.md](../gameplay/VELGRAD_BESTIARIUM.md): **Idle/Alert** + **Attack** + **Hurt** + **Death**.
...
Pro Boss-Encounter aus [../project-mgmt/WELT_AUFBAU.md](../project-mgmt/WELT_AUFBAU.md) §4 + ../lore/QUEST_BIBEL.md: **Spawn-Stinger** + **Phase-Transition** + **Kill-Stinger**.
